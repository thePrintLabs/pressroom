...
class WeDevs_Settings_API {
    ...
    /**
     * Displays a margin or pading field for a settings field
     *
     * @param array   $args settings field args
     */
    function callback_multiple_number( $args ) {
        ...
        $min = isset( $args['min'] ) ? $args['min'] : '';
        $max = isset( $args['max'] ) ? $args['max'] : '';
        $fields = isset( $args['fields'] ) ? $args['fields'] : false;
        $html = '';
        if( $fields ) {
          foreach( $fields as $key => $field ) {

            $val = isset($value[$key]) ? $value[$key] : '';
            $html .= sprintf( '<input type="%1$s" class="%2$s-text" id="%3$s[%4$s]['.$field.']" name="%3$s[%4$s][]" value="%5$s" min="%6$s" max="%7$s" placeholder="%8$s"/>', $type, $size, $args['section'], $args['id'], $val, $min, $max, $field );
          }
        }
