...
class PR_posts
{
  ...
  public function get_custom_metaboxes( $post ) {
    ...
    $e_meta = new PR_Metabox( 'sharing_metabox', __( 'Sharing', 'edition' ), 'normal', 'high', $post->ID );
		$e_meta->add_field( '_pr_sharing_url', __( 'Sharing Url', 'edition' ), __( 'Sharing url inside application. Leave it blank, for default value. ', 'pressroom' ), 'text', '', array( 'placeholder' => $placeholder ) );
